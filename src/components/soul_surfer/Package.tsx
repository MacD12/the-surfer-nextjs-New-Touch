...
const Package = () => {
  ...
  const tableTitle = t('soulSurferPackages.title');
  const packages = t('soulSurferPackages.packages', { returnObjects: true }) as Record<string, string>;
  const features = t('soulSurferPackages.features', { returnObjects: true }) as FeatureRow[];
  const accommodation = t('soulSurferPackages.accommodation', { returnObjects: true }) as AccommodationRow[];
                  ...
                  {cols.map((c, i) => (
                    <th
                      ...
                      className={`px-4 sm:px-6 py-4 text-center text-[10px] font-bold tracking-[0.18em] uppercase ${
                        i === 1 ? 'text-cyan-700 bg-cyan-50/50' : 'text-gray-500'
                      }`}
                    >
                      {c.label}
                    </th>
